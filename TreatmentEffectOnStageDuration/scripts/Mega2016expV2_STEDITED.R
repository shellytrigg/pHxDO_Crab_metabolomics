...
fit <- aov( duration~ treatment, data = subset(d, event2 == "J1ToJ2"))
summary(fit)

#Anova on time to J1ToJ2: with random MOATS effects
fitME <- lmer(duration~ treatment + (1|MOATS), data = subset(d, event2 == "J1ToJ2"))
summary(fitME)
...
treatment_list <- unlist(strsplit(d$treatment, "_"))
...
###plot durations x treatment
ggplot(d[grep("FREEZE", d$status),], aes(treatment, duration)) + geom_violin(aes(fill= treatment)) + geom_boxplot(aes(fill= treatment), width = 0.1) + theme(axis.text.x = element_blank(), strip.text.x = element_text(size = 7), strip.text.y = element_text(size = 4)) 


###aov of treatment x duration

durTreat <- aov(duration ~ treatment, data = d[grep("FREEZE", d$status),])

#summary(durTreat)
#             Df  Sum Sq  Mean Sq   F value    Pr(>F)    
#treatment     3    580   193.41     6.616      0.000223 ***
#  Residuals   427  12482   29.23    


durTreat_Tuk <- TukeyHSD(aov(duration ~ treatment, data = d[grep("FREEZE", d$status),]))

#Tukey multiple comparisons of means
#95% family-wise confidence level

#Fit: aov(formula = duration ~ treatment, data = d[grep("FREEZE", d$status), ])

#$treatment            diff         lwr      upr     p adj
#High_Low-High_High -0.3694444 -2.13528254 1.396394 0.9492859
#Low_High-High_High  2.3920940  0.59761024 4.186578 0.0035852
#Low_Low-High_High   1.8072679 -0.19627574 3.810812 0.0936529
#Low_High-High_Low   2.7615385  0.85429784 4.668779 0.0012193
#Low_Low-High_Low    2.1767123  0.07158008 4.281845 0.0395662
#Low_Low-Low_High   -0.5848261 -2.71404422 1.544392 0.8937472               

#calculate max duration
d$maxDur <- NA
for(i in 1:length(d$status)){
  if(d$status[i] == "FREEZE"){
    maxDur <-  d$duration[i]
  }
  d$maxDur[i] <- maxDur
  if(d$status[i] == "dead" ||  d$status[i] == "1J-dead" ||  d$status[i] == "M1" ||  d$status[i] == "m" ||  d$status[i] == "1J" ||  d$status[i] == "2J"){
    maxDur <- NA
  }
  d$maxDur[i] <- maxDur
}

FRZ_crabs <- d[grep("FREEZE", d$status),c(3,6,12)]
FRZ_crabs$cID <- paste(FRZ_crabs$MOATSjar, FRZ_crabs$date, sep = "_")
